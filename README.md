# Ball Balancing Stewart Platform 

<img width="536" height="382" alt="Balancer" src="https://github.com/user-attachments/assets/6a34a726-517d-4760-a780-7fc392ef41d7" />


A closed-loop control system that balances a ball on a 3-DOF Stewart platform using computer vision and PID control.

## How It Works
- Camera tracks ball position using HSV color detection
- Dual-axis PID controllers compute tilt corrections
- Inverse kinematics converts tilt angles to servo commands
- Three servos actuate the platform to keep the ball centered

## Quick Start
1. Upload main.ino to your arduino board
2. Run `python simple_cal.py` to calibrate color and geometry
3. Run `python main.py` to start the controller
4. Tune PID gains in real-time using the GUI sliders
